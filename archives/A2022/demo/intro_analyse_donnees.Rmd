...
Images (ce n'est _pas_ un plot) peuvent également être facilement incorporés avec `![](la_meilleure_image.png)`
...
```
La régression semble être en accord avec l'article, mais attention! Nous n'avons considéré que la corrélation entre les deux variables (correlation $\neq$ causation). Il y a beaucoup de possibilités qui pourraient générer les données. Par exemple, il pourrait y avoir une troisième variable qui a un effect _causal_ sur les deux variables que nous considérons ici.
